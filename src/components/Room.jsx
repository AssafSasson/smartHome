...
export default function Room(props) {
  ...
  // const switchOnOff = (index) => {

  //   if (props.deviceColor == 'red')
  //     props.setDeviceColor('green')

  //   else if (props.deviceColor == 'green')
  //     props.setDeviceColor('red');
  // }
  ...
  return (
    <div className='room-container'>
      ...
      <div className='all-devices'>{/* Show Devices inside the room */}
        {props.currentRoom.deviceArr.map((val, i) => {
          return <div onClick={() => {onDeviceClick(val)}} className='device-bg' index={i}><h2 style={{ backgroundColor: checkStatus(val.status)}}>{val.name}</h2></div>
        })}
      </div>
    </div>
  )
